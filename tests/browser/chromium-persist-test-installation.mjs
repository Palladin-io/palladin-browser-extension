...
// Run only on the harness's fresh profile, before it creates an account. Chromium browsers
// remove a CDP-only installation on restart. Its ordinary Extensions-page
...
export async function persistChromiumTestInstallation(context, extensionId, browserLabel = 'chrome') {
  assert(['chrome', 'edge', 'brave', 'opera'].includes(browserLabel), 'Tested browser UI adapter required')
  ...
  try {
    ...
    await page.goto(browserLabel === 'edge' ? 'edge://extensions' : 'chrome://extensions')
    let reload
