...
function fixture(initialize?: () => Promise<unknown>, onReady?: Parameters<typeof startFirefoxSharedUnlockBrowser>[3]) {
  ...
  const controller = startFirefoxSharedUnlockBrowser(environments, () => environments[0].apiUrl, initialize, onReady);
  const port = (tabId = 7) => {
    const connection = { name: SHARED_UNLOCK_BROWSER_PORT, sender: { id: "browser-extension@palladin.io", tab: { id: tabId, incognito: false }, frameId: bridgeFrame.frameId,
      documentId: bridgeFrame.documentId, origin: bridgeOrigin, url: bridgeUrl },
    ...
    api.runtime.onConnect.emit(connection as unknown as chrome.runtime.Port);
    return connection;
  };
...
describe("Firefox shared unlock runtime channel", () => {
  ...
  it.each(["navigation", "commit", "error"])("retires when the exact bridge has a new %s", async reason => {
    ...
    if (reason === "navigation") f.api.webNavigation.onBeforeNavigate.emit(bridgeNavigation);
    if (reason === "commit") f.api.webNavigation.onCommitted.emit(bridgeNavigation);
    if (reason === "error") f.api.webNavigation.onErrorOccurred.emit(bridgeNavigation);
    expect(p.disconnect).toHaveBeenCalled(); expect(f.controller.routes()).toHaveLength(0); f.controller.close();
  });
  it("does not resurrect a candidate accepted after its handshake deadline", async () => {
    const f = fixture(), p = f.port(), frames = await f.api.webNavigation.getAllFrames();
    let finish!: (value: typeof frames) => void;
    f.api.webNavigation.getAllFrames.mockImplementationOnce(() => new Promise(resolve => { finish = resolve; }));
    p.onMessage.emit(hello); await settle(); await vi.advanceTimersByTimeAsync(5000);
    finish(frames); await settle();
    expect(p.postMessage).not.toHaveBeenCalled(); expect(f.controller.routes()).toHaveLength(0); f.controller.close();
  });
  it("preserves adjacent operation ordering while serializing browser verification", async () => {
    ...
    const current = await f.api.webNavigation.getAllFrames();
    let resolve!: (value: typeof current) => void;
    f.api.webNavigation.getAllFrames.mockImplementationOnce(() => new Promise(r => { resolve = r; }));
  ...
  it("dispatches an operation only after rechecking the current browser document", async () => {
    ...
    expect(f.api.webNavigation.getAllFrames).toHaveBeenCalledTimes(3);
  ...
  it.each(["navigation", "overflow"])("cancels a pending operation browser check on %s", async reason => {
    ...
    const current = await f.api.webNavigation.getAllFrames();
    let resolve!: (value: typeof current) => void;
    f.api.webNavigation.getAllFrames.mockImplementation(() => new Promise(r => { resolve = r; }));
    const frame = { ...hello, type: "operation", channelId: route.channelId, documentBinding: route.documentBinding,
  ...
  it("uses current top-frame browser lookup and sends only the bounded ready frame", async () => {
    ...
    expect(f.api.webNavigation.getAllFrames).toHaveBeenCalledWith({ tabId: 7 });
    expect(p.postMessage).toHaveBeenCalledWith({ ...hello, type: "ready", webOrigin: environments[0].webOrigin,
      extensionId: "browser-extension@palladin.io", channelId: "A".repeat(43), documentBinding: `7/document-1/bridge-1/${"A".repeat(43)}` });
  ...
  it("retires at navigation start, denies the navigating document, then accepts a fresh committed document", async () => {
    ...
    const next = f.port();
    f.api.webNavigation.getAllFrames.mockResolvedValue([{ frameId: 0, parentFrameId: -1, url: environments[0].webOrigin, documentId: "document-2" }, { ...bridgeFrame, parentDocumentId: "document-2" }]);
  ...
  it("keeps admission closed until all overlapping server changes finish", async () => {
    ...
    expect(() => old.assertCurrent()).toThrow(); f.controller.close(); expect(f.api.runtime.onConnect.listeners.size).toBe(0);
  });
});
