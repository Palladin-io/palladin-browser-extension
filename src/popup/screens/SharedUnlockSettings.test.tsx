...
describe('shared popup and side-panel account preference', () => {
  ...
  it('does not expose a toggle when signed out and provides Polish instructions', async () => {
    ...
    render(<I18nProvider locale="pl"><SharedUnlockSettings {...f}><div>Pairing controls</div></SharedUnlockSettings></I18nProvider>)
    expect(await screen.findByRole('status')).toHaveTextContent('Zaloguj się')
    expect(screen.queryByText('Pairing controls')).not.toBeInTheDocument()
  ...
  it('ignores the previous screen save after its session changes', async () => {
    const f = setup(); render(<SharedUnlockSettings {...f}><div>Pairing controls</div></SharedUnlockSettings>)
